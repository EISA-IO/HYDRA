...
enum CredCatalog {
    static let all: [CredentialSpec] = [
        ...
        .init(key: "LEMONSQUEEZY_API_KEY",
              label: "Lemon Squeezy key",
              hint: "Merchant-of-record subscriptions — app.lemonsqueezy.com → Settings → API."),
        .init(key: "MOYASAR_SECRET_KEY",
              label: "Moyasar secret",
              hint: "sk_test_… / sk_live_… — KSA payments incl. mada + STC Pay (server-side only)."),
        ...
        .init(key: "FLY_API_TOKEN",
              label: "Fly.io token",
              hint: "fly tokens create deploy — used by flyctl for app deploys."),
        .init(key: "OPENROUTER_API_KEY",
              label: "OpenRouter key",
              hint: "One key → 300+ models incl. :free — openrouter.ai/keys."),
        .init(key: "OPENAI_API_KEY",
              label: "OpenAI API key",
              hint: "Optional for Hermes OpenAI API models; ChatGPT/Codex OAuth uses `hermes model`."),
        .init(key: "ANTHROPIC_API_KEY",
              label: "Anthropic API key",
              hint: "Optional for Hermes Claude models; OAuth setup is available through `hermes model`."),
