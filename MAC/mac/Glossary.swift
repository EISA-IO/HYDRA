...
enum Glossary {
    static let all: [GlossaryEntry] = {
        ...
        let H1 = "Hermes · 1  Getting started"
        add(H1, "★ hermes --tui", "Start the modern interactive agent UI; Hydra uses this (plus --yolo) for every Hermes terminal.")
        add(H1, "★ hermes -p <profile>", "Run an isolated profile with separate auth, skills, memory, and configuration.")
        add(H1, "★ hermes dashboard", "Open the full local GUI on http://localhost:9119 — config, auth, sessions, schedules, profiles, skills, tools, analytics, plugins. Hydra's Hermes tab has a one-click button for it.")
        add(H1, "hermes model", "Choose the primary provider and model; Hydra's Hermes tab can override both per launch.")
        add(H1, "hermes fallback", "Manage the provider/model chain used after rate limits, overloads, or connection failures.")
        add(H1, "hermes auth", "Add, list, reset, or remove pooled provider credentials.")
        add(H1, "hermes --provider / --model", "Per-launch overrides (openai-codex, anthropic, custom = local Ollama, openrouter).")
        add(H1, "hermes --continue", "Resume the most recent conversation in this profile.")
        add(H1, "hermes --yolo", "Skip per-command approval prompts — full access (Hydra launches Hermes this way).")

        let H2 = "Hermes · 2  Skills hub"
        add(H2, "★ hermes skills browse", "Explore the hub (88k+ skills) with paginated browsing across every registry.")
        add(H2, "hermes skills search <text>", "Search official, skills.sh, well-known, GitHub, ClawHub, and LobeHub registries.")
        add(H2, "hermes skills install <id|url>", "Install by registry ID (official/security/1password) or a direct SKILL.md URL.")
        add(H2, "hermes skills list", "Show installed skills with source, trust, and enabled status.")
        add(H2, "hermes skills inspect <id>", "Preview source, trust, metadata, and files before installing.")
        add(H2, "hermes skills config", "Enable or disable installed skills globally or per platform.")
        add(H2, "hermes skills check / update", "Check hub skills for updates and apply them.")
        add(H2, "hermes skills audit", "Audit installed skills for problems.")
        add(H2, "hermes skills uninstall / reset", "Remove a skill, or reset it to its installed state (--restore).")
        add(H2, "hermes skills opt-out", "Disable the bundled skill seeding (--remove --yes deletes them too).")
        add(H2, "hermes bundles create <name>", "Group several skills into one reusable command (list / show / delete).")

        let H3 = "Hermes · 3  Memory & context"
        add(H3, "★ hermes memory status", "Inspect built-in MEMORY.md / USER.md and the optional external memory provider.")
        add(H3, "hermes memory setup / off", "Attach or detach an external memory provider plugin; built-in memory is always on.")
        add(H3, "MEMORY.md / USER.md", "Built-in long-term memory and user profile, editable under the Hermes home's memories folder.")
        add(H3, ".hermes.md", "Per-project context file Hermes reads from the working folder.")
        add(H3, "★ hermes sessions browse", "Search, resume, rename, export, prune, or delete conversation history — your saved contexts.")
        add(H3, "hermes sessions list / export", "List past sessions or export one for sharing.")

        let H4 = "Hermes · 4  Automation & scheduling"
        add(H4, "★ hermes cron create \"<prompt>\"", "Schedule recurring agent or script tasks, optionally with --skill and project context.")
        add(H4, "hermes cron list --all", "List active and paused jobs; edit, pause, resume, run, or remove each schedule.")
        add(H4, "hermes cron status", "Check whether the scheduler is running.")
        add(H4, "hermes kanban", "Durable multi-profile task board — dependencies, claims, comments, retries, isolated workers.")
        add(H4, "hermes webhook subscribe", "React to supported external events such as GitHub issues and pull requests (list / remove).")
        add(H4, "hermes hooks", "Inspect, test, approve, or revoke project/profile shell hooks (list / test / revoke / doctor).")
        add(H4, "hermes gateway", "Run and manage the messaging gateway for connected chat platforms (start / stop / status).")
        add(H4, "hermes whatsapp / slack", "Set up supported messaging integrations and generated manifests.")

        let H5 = "Hermes · 5  Integrations & tools"
        add(H5, "hermes mcp", "Install and manage MCP servers, or expose Hermes itself as an MCP server.")
        add(H5, "hermes tools", "Choose toolsets for the CLI, gateway platforms, and scheduled jobs.")
        add(H5, "hermes plugins", "Install, update, enable, disable, and remove Hermes runtime plugins.")
        add(H5, "hermes curator", "Run or pause background skill maintenance and pin skills that must not change.")
        add(H5, "hermes computer-use", "Manage the Computer Use backend where the operating system supports it.")
        add(H5, "hermes lsp / acp", "Manage language servers or run Hermes as an Agent Client Protocol server.")
        add(H5, "hermes proxy", "Run a local OpenAI-compatible proxy for supported OAuth-backed providers.")

        let H6 = "Hermes · 6  Maintenance & diagnostics"
        add(H6, "★ hermes status --all", "Show component, provider, gateway, schedule, and tool health with secrets redacted (--deep for more).")
        add(H6, "★ hermes doctor --fix", "Diagnose and repair configuration, dependencies, providers, skills, and local tooling.")
        add(H6, "hermes update --backup --yes", "Back up the active home, then update through the supported lifecycle command.")
        add(H6, "hermes backup / import", "Back up the Hermes home to a zip and restore it through the supported path.")
        add(H6, "hermes checkpoints", "Inspect, prune, or clear saved tool-execution checkpoints.")
        add(H6, "hermes logs --since 1h", "Filter agent, error, gateway, cron, and component logs.")
        add(H6, "hermes insights", "Review usage insights and analytics by session and model.")
        add(H6, "hermes config", "View or set supported configuration keys (show / set / edit).")
        add(H6, "hermes dump / debug share", "Create a redacted support summary or diagnostic report.")
