#!/usr/bin/env bash
# Claude Manager (macOS) launcher.
#   • If the native app "Claude Manager.app" is present, open it.
...
# Double-click in Finder. First run may need: chmod +x Claude-Manager-Mac.command
set -uo pipefail
...
if command -v swiftc >/dev/null 2>&1 && [ -d "$HERE/mac" ]; then
  osascript -e 'display notification "Building Claude Manager (first run)…" with title "Claude Manager"' >/dev/null 2>&1
