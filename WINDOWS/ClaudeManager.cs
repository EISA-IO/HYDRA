...
// Claude Manager - standalone launcher + skills manager + reference for the Claude CLI.
class ClaudeManager : Form
{
    ...
    static void Main(string[] args)
    {
        ...
        {
            MessageBox.Show("Claude Manager could not start:\n\n" + ex.Message,
                "Claude Manager", MessageBoxButtons.OK, MessageBoxIcon.Error);
        }
    }
    ...
    void SpawnDemoTerminal(string label, bool hr, bool rt, bool cv)
    {
        ...
        string inner = "title Claude " + id + " & cls & echo ================================================"
            ...
            + " & echo   This console is running INSIDE Claude Manager (not external)."
            + " & echo ================================================ & echo.";
        try
        {
    ...
    void InitAlerts()
    {
        ...
        tray = new NotifyIcon { Icon = AppIcon() ?? SystemIcons.Application, Visible = true, Text = "Claude Manager" };
    ...
    bool EnsureProxy()
    {
        ...
        catch
        {
            MessageBox.Show("Headroom is not installed / not on PATH.\n\nInstall it, or uncheck 'Route through Headroom'.",
                "Claude Manager", MessageBoxButtons.OK, MessageBoxIcon.Warning);
            return false;
        }
    ...
    void StartClaude(string folder)
    {
        ...
        {
            MessageBox.Show("Folder not found:\n" + folder, "Claude Manager", MessageBoxButtons.OK, MessageBoxIcon.Warning);
            return;
        }
    ...
    List<GEntry> BuildGlossary()
    {
        ...
        g.Add(new GEntry(X, "codex -C <dir>", "Start Codex with an explicit working root. Claude Manager uses this for every Codex terminal."));
        g.Add(new GEntry(X, "--model <id>", "Choose a ChatGPT model for Codex, e.g. ChatGPT 5.6 maps to gpt-5.6."));
        g.Add(new GEntry(X, "--dangerously-bypass-approvals-and-sandbox", "YOLO mode: no approvals and no sandbox. Claude Manager starts Codex terminals this way."));
        ...
        g.Add(new GEntry(X, "~/.codex/AGENTS.md", "Global Codex instructions. Claude Manager writes RTK/Caveman guidance here."));
        g.Add(new GEntry(X, "~/.agents/skills", "Codex/ChatGPT user skills folder. Claude Manager mirrors bundled/imported skills here."));
        g.Add(new GEntry(X, "rtk init -g --codex", "Install RTK guidance for Codex so shell commands use token-filtered rtk output."));
        g.Add(new GEntry(X, "CODEX_HOME", "Points Codex at its config/state directory; Claude Manager sets it to ~/.codex."));
        ...
        string M = "Claude Manager (this app)";
    ...
    // (/resource at build time) so a standalone ClaudeManager.exe is fully self-contained.
    ...
    void BuildUi()
    {
        ...
        var hTitle = new Label { Text = "Claude Manager", AutoSize = true, Location = new Point(64, 8),
    ...
    void BuildTitleBar(int h)
    {
        ...
        var cap = new Label { Text = "Claude Manager", AutoSize = true, ForeColor = TextDim,
...
try {
  ...
  Write-Host 'OK  RTK downloaded to ~/.local/bin/rtk.exe'
    ...
    void NewTerminal(string presetFolder)
    {
        ...
        {
            MessageBox.Show("Failed to start terminal:\n" + ex.Message, "Claude Manager", MessageBoxButtons.OK, MessageBoxIcon.Error);
            return;
        }
    ...
    {
        try { if (tray != null) tray.ShowBalloonTip(6000, "Claude Manager — " + title, msg, ToolTipIcon.Info); } catch { }
    }
    ...
    bool SaasDeployDirValid()
    {
        ...
        {
            MessageBox.Show("Create your app first (Stage 1), or point 'Parent folder' at an existing project (a folder with package.json / .git). To protect you, these actions never run on your home folder.", "Claude Manager");
            return false;
        }
        return true;
    }
    ...
    void SaasDeployNow()
    {
        if (!SaasDeployDirValid()) return;
        string exe, install, login; SaasTargetTool(out exe, out install, out login);
        if (!(OnPath(exe) || OnPath(exe + ".cmd"))) { MessageBox.Show("Not connected — run step 1 (Connect your host) first; it installs the " + exe + " CLI and signs you in.", "Claude Manager"); return; }
    ...
    void SaasCheckWasp()
    {
        ...
        if (MessageBox.Show("Install the Wasp CLI inside WSL now?\n\nRuns:  curl -sSL https://get.wasp.sh/installer.sh | sh", "Claude Manager", MessageBoxButtons.OKCancel, MessageBoxIcon.Question) != DialogResult.OK) return;
        try
        {
    ...
    void SaasCreate()
    {
        ...
        if (name.Length == 0 || parent.Length == 0 || !Directory.Exists(parent)) { MessageBox.Show("Set a valid parent folder and app name first.", "Claude Manager"); return; }
        if (Directory.Exists(SaasAppDir())) { MessageBox.Show("That app folder already exists:\n" + SaasAppDir() + "\n\nPick a new name or delete it first.", "Claude Manager"); return; }
...
";
    ...
    {
        string app = SaasAppDir();
        if (!Directory.Exists(app)) { MessageBox.Show("App folder not found. Run step 2 first:\n" + app, "Claude Manager"); return; }
        if ((saasPitch.Text ?? "").Trim().Length == 0) { MessageBox.Show("Add a one-line pitch first so Claude understands the vision.", "Claude Manager"); return; }
        try
        {
            File.WriteAllText(Path.Combine(app, "PLAYBOOK.md"), PlaybookDoc());
            File.WriteAllText(Path.Combine(app, "VISION.md"), BuildVisionDoc());
    ...
    {
        string app = SaasAppDir();
        if (!Directory.Exists(app)) { MessageBox.Show("App folder not found. Run step 2 first.", "Claude Manager"); return; }
        bool haveWasp = OnPath("wasp.exe") || OnPath("wasp");
        try
        {
