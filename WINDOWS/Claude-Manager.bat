...
REM Launch the FULL Claude Manager GUI (modern tabbed UI: Workspace / Settings /
...
set "HERE=%~dp0"
set "EXE=%HERE%ClaudeManager.exe"
set "SRC=%HERE%ClaudeManager.cs"
...
if exist "%EXE%" if exist "%SRC%" (
    ...
    if /i "!NEWEST!"=="ClaudeManager.cs" set "NEEDBUILD=1"
)
...
if defined NEEDBUILD (
    if exist "%SRC%" (
        ...
        if exist "!CSC!" (
            echo Building Claude Manager, one moment...
            set "RES="
...
if exist "%EXE%" (
    ...
    start "" powershell.exe -NoProfile -ExecutionPolicy Bypass -STA -WindowStyle Hidden -File "%HERE%ClaudeManager.ps1"
)
endlocal
